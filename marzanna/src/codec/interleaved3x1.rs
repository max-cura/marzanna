...
impl Codec for Interleaved3x1 {
    ...
    fn decode(&mut self, msg: &[bool], buf: &mut BytesMut) {
        assert!(
            ...
            "msg length must be a multiple of 3x8=40"
        );

        let pre_bits = msg.len() / 3;

        buf.reserve(msg.len() / 3 / 8);
